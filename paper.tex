...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Single-token addressing (architectural trade-off).} The trace addresses entities by the Q-vector of a single BPE token, which functions as a \emph{pointer} into the value trace rather than a semantic encoding of the entity. This is a deliberate trade-off: token-identity addressing provides cross-session stability and deterministic write/read alignment through pattern separation's top-k step, but produces collisions when multiple entities share a first BPE token (40.7\% of HotpotQA bridge entities). Hashed trace banks with best-bank scan resolve these collisions without modifying Q (+36pp at batch 15, maintaining ${\geq}98.7\%$ accuracy), while the host LM handles multi-token completion auto-regressively from the retrieved first token. Direct multi-token Q addressing---composing a single Q-vector from multiple entity tokens---is incompatible with the top-k sparsification step: even $\varepsilon{=}0.01$ additive perturbation changes which expanded dimensions survive top-k, breaking write/read alignment. This reflects a fundamental property of discrete sparsification rather than an engineering limitation, and motivates future investigation of continuous relaxations (e.g., Sparsemax or straight-through estimators) that preserve sparsity while admitting smooth Q-space interpolation.
