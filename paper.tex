...
\begin{enumerate}[leftmargin=*]
  \item \textbf{A modular persistent memory architecture} comprising seven bio-inspired, independently validated components that compose orthogonally, enabling cross-session knowledge retention, paraphrase resolution, and multi-hop retrieval for frozen LLMs.
  ...
  \item \textbf{Multi-hop retrieval validated on real-world facts.} A two-hop trace chain achieves 100\% end-to-end accuracy on 841 HotpotQA \citep{yang2018hotpotqa} bridge questions under oracle-supported evaluation, with zero new parameters---intermediate results are decoded from trace via argmax and re-encoded as new queries. Best-bank scan retrieval maintains ${\geq}98.5\%$ accuracy with up to 15 simultaneous question batches, resolving first-token collisions that affect 40.7\% of bridge entities.

  \item \textbf{Real-world entity vocabularies.} On the LAMA T-REx benchmark (2,034 Wikidata facts, 32 relations), cross-context trace retrieval achieves 81--94\% accuracy with oracle-assigned concept words, exceeding GPT-2's in-context baseline by +50--60pp. This validates storage capacity on realistic entity vocabularies (1,014 unique tokens), though the concept-word mapping simplifies the task to associative storage rather than open-domain knowledge probing.
...
We validate the architecture through systematic experiments across three model scales and two architectures, following a progression from individual components to multi-hop retrieval on a real benchmark.
...
To test storage capacity on realistic entity vocabularies beyond synthetic tasks, we evaluate on the LAMA T-REx benchmark \citep{petroni2019language}. We use LAMA as a source of real Wikidata facts with diverse entity tokens, rather than as an end-to-end knowledge probing test---the concept-word assignment required by our architecture simplifies the task relative to standard LAMA evaluation (see caveats below).
...
\begin{table}[t]
  \centering
  \caption{LAMA T-REx evaluation: cross-context retrieval accuracy across fact counts (100 episodes, bootstrap 95\% CIs, oracle-assigned concept words). The trace exceeds GPT-2's in-context baseline by +50--60pp at $n{\geq}3$.}
  \label{tab:lama}
  \begin{tabular}{@{}lccc@{}}
    \toprule
...
\textbf{Caveats.}\quad This evaluation tests the trace mechanism's associative storage capacity on real-world knowledge, not end-to-end knowledge probing. Each T-REx relation is assigned a fixed concept word (e.g., P36 $\to$ ``capital''), reducing the task to: store(``capital'', ``Paris''), query(``capital''). The LM is still needed---trace injection competes with GPT-2's pretrained prior, which actively biases toward its own knowledge---but the concept-word assignment is oracle-provided. Additionally, the single-token constraint limits coverage to ${\sim}6\%$ of LAMA T-REx. These results validate storage capacity on real entity vocabularies (1,014 unique tokens vs.\ ${\sim}100$ in synthetic tasks), but do not demonstrate open-domain knowledge acquisition.
...
\textbf{Memory footprint.}\quad The trace stores a fixed $d \times d$ matrix per head per bank ($64 \times 64 \times 8 \times 16 = 524\text{K}$ parameters), independent of the number of stored facts. kNN-LM stores one $d_{\text{model}}$-dimensional vector per token position, growing as $O(N \cdot L)$ where $N$ is the number of facts and $L$ the average token length. At $n{=}100$ facts (${\sim}1{,}000$ tokens), kNN-LM requires ${\sim}768\text{K}$ floats---comparable to the trace. But at $n{=}1{,}000$ facts, kNN-LM grows to $7.7\text{M}$ floats while the trace remains at $524\text{K}$. Retrieval cost also differs: the trace computes a single $\mathbf{Q} \times \mathbf{T}_v$ matrix-vector product ($O(d^2)$, constant), while kNN-LM requires a nearest-neighbor search over all stored vectors ($O(N \cdot d)$, linear in datastore size).
...
\begin{table}[t]
  \centering
  \caption{HotpotQA 2-hop evaluation (841 bridge questions, PS $8\times/k{=}16$, 32 banks). Oracle-supported: supporting facts provided, single-token answers only (${\sim}16\%$ of bridge questions); not comparable to end-to-end QA leaderboard entries. Per-question: trace reset between questions. Batched: $N$ questions share a single trace, 50 random batches. Best-bank scan reads all banks and picks the highest-confidence answer.}
  \label{tab:hotpotqa}
  \begin{tabular}{@{}cccccc@{}}
    \toprule
    Mode & $N$ & Total facts & Hop-1 & E2E (no banks) & E2E (best-bank) \\
    \midrule
    Per-question & 1 & 2 & 100\% & \textbf{100\%} (841/841) & --- \\
    \midrule
    Batched & 5 & 10 & 100\% & 98.8\% & \textbf{99.6\%} \\
    Batched & 8 & 16 & 99.2\% & 92.5\% & \textbf{99.8\%} \\
    Batched & 10 & 20 & 94.6\% & 85.6\% & \textbf{98.8\%} \\
    Batched & 15 & 30 & 80.5\% & 63.2\% & \textbf{98.5\%} \\
    \bottomrule
  \end{tabular}
\end{table}

The primary bottleneck without banks is bridge-entity first-token collision: 40.7\% of questions share a bridge entity first BPE token (e.g., ``The''), creating identical Q-vectors at hop-2. Hashed trace banks (Section~\ref{sec:banks}) resolve this: with 32 banks and a best-bank scan at retrieval (reading all banks and selecting the highest-confidence answer), end-to-end accuracy improves from 63.2\% to 98.5\% at batch size 15 (+35pp). Notably, modifying the Q-vector itself (composite Q from multiple entity tokens) is incompatible with pattern separation's top-k sparsification---even tiny perturbations change which expanded dimensions survive top-k, breaking write/read address alignment. Banks solve the collision without altering Q, preserving pattern separation compatibility.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Single-token entities (partially addressed).} Entity values must tokenize to exactly one BPE token. Multi-hop chains use first-token addressing for multi-token bridge entities (Section~\ref{sec:multihop_exp}), which produces collisions (40.7\% of HotpotQA bridge entities share a first token like ``The''). Hashed trace banks with best-bank scan resolve 95\%+ of collisions (+35pp at batch 15), but direct multi-token Q addressing remains incompatible with pattern separation's top-k step.
