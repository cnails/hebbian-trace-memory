...
\begin{enumerate}[leftmargin=*]
  \item \textbf{A modular persistent memory architecture} comprising seven bio-inspired, independently validated components that compose orthogonally, enabling cross-session knowledge retention, paraphrase resolution, and multi-hop retrieval for frozen LLMs. Hash-routed trace banks extend capacity scaling: with 16 banks, accuracy remains ${\geq}99\%$ through 100 simultaneously stored facts (vs.\ 35.6\% without banks), yielding an essentially flat scaling curve with zero new parameters.
  ...
  \item \textbf{Real-world validation on two public benchmarks.} A two-hop trace chain achieves 100\% end-to-end accuracy on 4,878 HotpotQA \citep{yang2018hotpotqa} bridge questions under oracle-supported evaluation, maintaining ${\geq}98.7\%$ with up to 15 simultaneous question batches via best-bank scan retrieval. On the LAMA T-REx benchmark (2,034 Wikidata facts, 40 relations), cross-context trace retrieval achieves 81--94\% accuracy---with automated concept-word assignment matching oracle curation within confidence intervals---exceeding GPT-2's in-context baseline by +50--60pp on a realistic entity vocabulary (1,014--1,107 unique tokens).
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lll@{}}
    \toprule
    Component & Effect ($n{=}10$) & Mechanism \\
    \midrule
    Pattern separation (\S\ref{sec:pattern_sep}) & +9.3pp cross-context & Sparse Q reduces interference \\
    Adaptive alpha & +16pp Hebbian, +2pp cross & Score trace normalization \\
    CLS decay (\S\ref{sec:cls}) & +7.4pp overall (updates) & Fast/slow head specialization \\
    Selective erasure (\S3.3) & +11.7pp overall (updates) & Anti-Hebbian old-value removal \\
    Concept injection & +90pp Tier~2 templates & Semantic Q override \\
    Dual gates (\S3.2) & +46pp noisy paragraphs & Semantic filtering of storage \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{HotpotQA benchmark.}\quad We evaluate on real multi-hop questions from HotpotQA \citep{yang2018hotpotqa}, filtering the validation set to bridge-type questions with identifiable bridge entities and non-degenerate answers (bridge $\neq$ answer). Multi-token answers are evaluated via first BPE token match. From 5,918 bridge questions, 4,878 pass our filters (${\sim}82\%$ of bridge questions). Bridge entities are identified from the dataset's supporting fact annotations.

\begin{table}[t]
  \centering
  \caption{HotpotQA 2-hop evaluation (4,878 bridge questions, PS $8\times/k{=}16$, 16 banks). Oracle-supported: supporting facts provided, first BPE token answer match (${\sim}82\%$ of bridge questions); not comparable to end-to-end QA leaderboard entries. Per-question: trace reset between questions. Batched: $N$ questions share a single trace, 50 random batches. Best-bank scan reads all banks and picks the highest-confidence answer.}
  \label{tab:hotpotqa}
  \begin{tabular}{@{}cccccc@{}}
    \toprule
    Mode & $N$ & Total facts & Hop-1 & E2E (no banks) & E2E (best-bank) \\
    \midrule
    Per-question & 1 & 2 & 100\% & \textbf{100\%} (4,878/4,878) & --- \\
    \midrule
    Batched & 5 & 10 & 100\% & 99.6\% & \textbf{99.6\%} \\
    Batched & 8 & 16 & 98.8\% & 97.0\% & \textbf{98.8\%} \\
    Batched & 10 & 20 & 96.6\% & 90.8\% & \textbf{98.8\%} \\
    Batched & 15 & 30 & 81.9\% & 67.6\% & \textbf{98.7\%} \\
    \bottomrule
  \end{tabular}
\end{table}

The primary bottleneck without banks is bridge-entity first-token collision: 71.0\% of questions share a bridge entity first BPE token (e.g., ``The''), creating identical Q-vectors at hop-2. Hashed trace banks (Section~\ref{sec:banks}) resolve this: with 16 banks and a best-bank scan at retrieval (reading all banks and selecting the highest-confidence answer), end-to-end accuracy improves from 67.6\% to 98.7\% at batch size 15 (+31pp). Notably, modifying the Q-vector itself (composite Q from multiple entity tokens) is incompatible with pattern separation's top-k sparsification---even tiny perturbations change which expanded dimensions survive top-k, breaking write/read address alignment. Banks solve the collision without altering Q, preserving pattern separation compatibility.

We emphasize that this evaluation tests the trace mechanism's multi-hop capability on real-world facts, not end-to-end QA performance---oracle supporting facts and first-token answer matching mean these results are not directly comparable to HotpotQA leaderboard entries, which must additionally solve retrieval from 5M+ paragraphs and open-ended answer generation.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Single-token entities (partially addressed).} Entity values must tokenize to exactly one BPE token. Multi-hop chains use first-token addressing for multi-token bridge entities (Section~\ref{sec:multihop_exp}), which produces collisions (71.0\% of 4,878 HotpotQA bridge entities share a first token like ``The''). Hashed trace banks with best-bank scan resolve 95\%+ of collisions (+31pp at batch 15), but direct multi-token Q addressing remains incompatible with pattern separation's top-k step.
  ...
  \item \textbf{Benchmark coverage (partially addressed).} The HotpotQA evaluation (4,878 bridge questions, ${\sim}82\%$ of bridge questions, Section~\ref{sec:multihop_exp}) covers the majority of bridge-type questions via first-token answer matching. However, it uses oracle supporting facts and oracle bridge entity identification. End-to-end QA with open retrieval requires solving paragraph selection and bridge entity reasoning, which are outside the scope of the trace mechanism itself.
\end{enumerate}
...
We have presented the Hebbian Trace Module, a bio-inspired external memory system that provides persistent, updatable, and compositional memory for frozen language models. Through systematic development across three model scales, we showed that seven composable bio-inspired mechanisms---pattern separation, heteroassociative and autoassociative Hebbian storage, dual semantic gating, reconsolidation erasure, complementary learning systems, and hippocampal replay---together with hash-routed trace banks for capacity scaling, can be combined to achieve 98--99\% mean recall across 15 sessions, scale to ${\geq}99\%$ accuracy at 100 simultaneously stored facts, resolve paraphrased queries (+83pp on misaligned queries), exceed GPT-2's in-context baseline by +50--60pp on LAMA T-REx facts (1,014--1,107 unique entity tokens, automated concept assignment equivalent to oracle curation), and perform oracle-supported multi-hop retrieval at 100\% accuracy on 4,878 HotpotQA bridge questions---maintaining ${\geq}98.7\%$ with 15 simultaneous question batches via best-bank scan retrieval---all with ${\sim}1$M additional parameters. Each mechanism is independently motivated by hippocampal neuroscience, independently validated through ablation, and composes orthogonally with the others. Comparison with the kNN-LM baseline---the closest existing approach for frozen-model memory augmentation---demonstrates that the trace's context-free addressing provides a +63pp advantage on cross-context retrieval at $n{=}5$, where storage and query use different phrasing. Zero-shot cross-architecture transfer to Phi-2 (2.7B) achieves 92--100\% accuracy with performance that improves with model size, demonstrating architecture-agnostic generalization. An LLM-based extraction pipeline achieves 94\% F1 on unconstrained free-form text, and a multi-turn dialogue system demonstrates conversational integration. Significant limitations remain---notably single-token entity addressing and open-domain relation types---but our results suggest that the computational principles underlying biological memory systems offer viable inductive biases for adding persistent, compositional memory to pretrained language models without modifying their weights.
